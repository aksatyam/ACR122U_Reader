                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (nextStates == Reader.CARD_PRESENT) {
                            logMsg("Card Status : "+ stateStrings[nextStates]);
                           ...
                           // Load Default Authentication Key
                            byte[] authBuffer = new byte[] {(byte) 0xFF, (byte) 0x82,(byte) 0x00,(byte) 0x00,(byte) 0x06,
                            ...
                            try {
                                ...
                                // Getting UID
                                byteCount = mReader.control(slotNos, Reader.IOCTL_CCID_ESCAPE, sendBuffer, sendBuffer.length, receiveBuffer, receiveBuffer.length);
                                // Load default Authentication key
                                authKeyResponse = mReader.transmit(slotNos, authBuffer, authBuffer.length, authResponse, authResponse.length);

                                if (toHexString(authResponse).contains("90")) {
                                    for(int i=4; i< 8; i++) {
                                        getKeys(i);
                                        mReader.transmit(slotNos, authenticateBlock, authenticateBlock.length, authenticateBlockReponse, authenticateBlockReponse.length);
                                        if (toHexString(authenticateBlockReponse).contains("90")){
                                            mReader.transmit(slotNos, readBlock, readBlock.length, readBlockReponse, readBlockReponse.length);
                                            if (i == 4) {
